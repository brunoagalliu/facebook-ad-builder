...
const NanoBananaGenerationStep = ({ copyData, selectedTemplate, onImagesGenerated, onNext, onBack }) => {
    ...
    return (
        <div>
            ...
            <p className="text-ink-secondary mb-8">Create stunning visuals using Nano Banana Pro AI.</p>

            {/* Generation Controls */}
            <div className="bg-surface border border-border rounded-xl p-6 mb-8">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h3 className="font-bold text-ink">Generation Settings</h3>
                        <p className="text-sm text-ink-tertiary">Based on your copy and selected template</p>
                    </div>
                    <button
                ...
                {/* Context Summary */}
                <div className="flex gap-4 text-sm text-ink-secondary bg-surface-hover p-3 rounded-lg">
            ...
            {generatedImages.length > 0 && (
                <div className="mb-8">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-sm text-ink-tertiary">{selectedImages.length} selected</span>
                    </div>
                            ...
                            >
                                ...
                                <div className={`absolute inset-0 bg-black/20 transition-opacity ${selectedImages.find(img => img.id === image.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                                    ...
                                    <div className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center ${selectedImages.find(img => img.id === image.id)
                                            ...
                                            : 'bg-surface/80 text-ink-tertiary'
                                        }`}>
                                        <Check size={14} />
                                    </div>
                                </div>
            ...
            {generatedImages.length === 0 && !generating && (
                <div className="text-center py-16 bg-surface-hover rounded-xl border-2 border-dashed border-border mb-8">
                    <div className="w-16 h-16 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">
                        <ImageIcon size={32} />
                    </div>
                    <h3 className="text-lg font-medium text-ink mb-1">Ready to Generate</h3>
                    <p className="text-ink-tertiary">Click the generate button to create images with Nano Banana Pro.</p>
                </div>
            )}
            ...
            <div className="flex justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:bg-border disabled:cursor-not-allowed"
                >
