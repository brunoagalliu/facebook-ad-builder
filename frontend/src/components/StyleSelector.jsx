        ...
        <div className="space-y-6">
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-border rounded-lg focus:ring-2 focus:ring-brand-600 focus:border-transparent"
                />
            </div>
                ...
                {filteredStyles.map((style) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Style Name */}
                            <h4 className="font-bold text-ink mb-2 text-lg">{style.name}</h4>

                            {/* Description */}
                            <p className="text-sm text-ink-secondary mb-3">{style.description}</p>

                            {/* Visual Layout */}
                            <div className="mb-3">
                                <span className="text-xs font-medium text-ink-tertiary">Layout:</span>
                                <p className="text-sm text-ink-secondary">{style.visualLayout}</p>
                            </div>

                            {/* Best For Tags */}
                            <div className="mb-3">
                                <span className="text-xs font-medium text-ink-tertiary block mb-1">Best For:</span>
                                <div className="flex flex-wrap gap-1">
                                    {style.bestFor.map((industry) => (
                                        <span
                                            key={industry}
                                            className="px-2 py-0.5 bg-surface-hover text-ink-secondary text-xs rounded"
                                        >
                                            {industry}
                                        </span>
                                    ))}
                            ...
                            {isHovered && (
                                <div className="mt-4 pt-4 border-t border-border animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div>
                                            <span className="text-xs font-medium text-ink-tertiary block mb-1">Psychology:</span>
                                            <p className="text-sm text-ink-secondary italic">{style.psychology}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
            ...
            {filteredStyles.length === 0 && (
                <div className="text-center py-12">
                    <Search className="mx-auto text-ink-tertiary mb-4" size={48} />
                    <h3 className="text-lg font-bold text-ink mb-2">No styles found</h3>
                    <p className="text-ink-secondary">Try adjusting your search or browse all categories</p>
                </div>
            )}
        </div>
    );
