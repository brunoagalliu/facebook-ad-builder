    ...
    return (
        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    {templates.map((template) => (
                        ...
                        >
                            ...
                            <div className="p-4">
                                <h3 className="font-bold text-ink mb-1">{template.name}</h3>
                                <p className="text-sm text-ink-tertiary mb-3">{template.description}</p>
                                <div className="flex flex-wrap gap-2">
                                    {template.tags.map(tag => (
                                        <span key={tag} className="text-xs bg-surface-hover text-ink-secondary px-2 py-1 rounded-full">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-sm text-ink-tertiary line-clamp-1">{template.description}</p>
                                    </div>
                                    {selectedTemplate?.id === template.id && (
            ...
            <div className="flex justify-between mt-8">
                <button
                    onClick={onBack}
                    className="px-6 py-2 bg-border text-ink-secondary rounded-lg hover:bg-border transition-colors"
                >
                    Back
                </button>
                <button
