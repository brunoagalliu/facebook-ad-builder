...
const Wizard = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto">
            {/* Header */}
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-ink">Video Ad Builder</h1>
                <p className="text-ink-secondary mt-1">Create video ads with AI-generated content</p>
            </div>
            ...
            <div className="mb-10">
                <div className="flex items-center justify-between relative">
                    ...
                    {steps.map((s) => {
                        ...
                        return (
                            <div key={s.id} className="flex flex-col items-center bg-surface-hover px-2">
                                <div
                                ...
                                <span className={`text-xs font-medium hidden md:block ${isActive ? 'text-blue-600' : 'text-ink-tertiary'}`}>
                                    {s.name}
                                </span>
                            </div>
                        );
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex bg-surface-hover p-1 rounded-lg">
                                <button
                                    ...
                                    className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-surface shadow-sm text-blue-600' : 'text-ink-tertiary hover:text-ink-secondary'}`}
                                >
                                    <List size={20} />
                                </button>
                                <button
                                    onClick={() => setViewMode('grid')}
                                    className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-surface shadow-sm text-blue-600' : 'text-ink-tertiary hover:text-ink-secondary'}`}
                                >
                                    <LayoutGrid size={20} />
                                </button>
                            </div>
                        ...
                        {brands.length === 0 ? (
                            <div className="text-center py-12">
                                <p className="text-ink-tertiary mb-4">No brands found. Create a brand to get started.</p>
                                                ...
                                                <div key={brand.id} className="border-2 rounded-xl overflow-hidden transition-all">
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <span className="font-bold text-ink block">{brand.name}</span>
                                                    ...
                                                    {isExpanded && brandProfiles.length > 0 && (
                                                        <div className="bg-surface-hover border-t border-border p-4">
                                                            <h4 className="text-xs font-semibold text-ink-tertiary uppercase mb-2">Customer Profiles</h4>
                                                            <div className="space-y-2">
                                                                {brandProfiles.map(profile => (
                                                                    <div
                                                                        key={profile.id}
                                                                        className="bg-surface p-3 rounded-lg border border-border text-sm"
                                                                    >
                                                                        <div className="font-medium text-ink mb-1">{profile.name}</div>
